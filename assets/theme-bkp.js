    ...
    class CartDrawer {
        ...
        init() {
            (this.settings = settings),
                ...
                (this.cartSavingAdditional = this.document.querySelector('[cartSavingAdditional]')),
        ...
        cartEvents() {
            ...
            this.document.querySelectorAll(e.settings.elements.cartItemRemove).forEach((t) => {
                t.addEventListener("click", function (t) {
                  $(this).parent().find('.loader1').css('display','block'); 
        ...
        buildTotalPrice(e) {
          var discount_5 = 0, discount_10 = 0, discount_20 = 0;
            if (
                ...
                (this.cartTotal.innerHTML = themeCurrency.formatMoney(e.total_price, this.settings.formatMoney)),
                 ...
                 // console.log(">>>this.cartSavingPrepaid ",e.total_discount),
                  ...
                  if(discount.title == 'Additional 5% discount on online payment mode.'){
                    discount_5 = discount_5 + discount.amount
                  }
                  if(discount.title == '25% Off'){
                    discount_10 = discount_10 + discount.amount
                  }
                   if(discount.title == '30% Off'){
                    discount_20 = discount_20 + discount.amount
                  } 
                  })
                 }),
               
                 (discount_20 > 0 ? (this.cartSavingAdditional.previousElementSibling.innerHTML = "Additional 30% Discount", this.cartSavingAdditional.parentNode.style.display = "flex", this.cartSavingAdditional.innerHTML = themeCurrency.formatMoney(discount_20, this.settings.formatMoney)):(this.cartSavingAdditional.parentNode.style.display = "none",
                 (discount_10 > 0 ? (this.cartSavingAdditional.previousElementSibling.innerHTML = "Additional 25% Discount", this.cartSavingAdditional.parentNode.style.display = "flex", this.cartSavingAdditional.innerHTML = themeCurrency.formatMoney(discount_10, this.settings.formatMoney)) : this.cartSavingAdditional.parentNode.style.display = "none" ))),
                (this.cartSavingPrepaid.innerHTML = themeCurrency.formatMoney(discount_5, this.settings.formatMoney)),
...
function addGwbProductCart(result) {
  ...
  $('.item__counts>span').text(_totalItem);
  // console.log(">>>>>", _totalPrice);
  let itemCount = _totalItem;
  if(_totalPrice > 0 ){
        if(result.items.some(e => e.id == '43324853485805')){
          itemCount = itemCount - 0;
        }if(result.items.some(e => e.id == '43324857975021')){
           itemCount = itemCount - 1;
        }
    // if(result.items.some(e => e.id == '43106135769325')){
    //        itemCount = itemCount - 1;
    //     }
        
    
    if(_totalPrice > 60000 && _totalPrice < 99900){
      // console.log(">>>>>result ", result); 
      if(result.items.some(e => e.id == '43324853485805') && result.item_count == 1){
        free();
      }else if(result.items.some(e => e.id == '43324853485805') && result.items.some(e => e.id == '43324857975021') && result.item_count == 2){
        free();
      }else{
        let variants = {
            "43324853485805":0,
        ...
        //      "43106135769325":1
        //     }
        //   sessionStorage.setItem('freeToner', 'added');
        // }else{
        //  if(sessionStorage.getItem('freeToner') != null){
        //    sessionStorage.removeItem('freeToner');
        //    variants = {
        //    "43324853485805":0,
        //     "43324857975021":0,
        //      "43106135769325":0
        ...
        //  }
        // }
        // console.log(">>>freeToner",sessionStorage.getItem('freeToner'));
        $.ajax({
         type: 'POST',
         url: '/cart/update.js', 
         data: {
          ...
          success: function(data){
           $('.item__counts>span').text(data.item_count);
  ...
  .then(result =>  ship_update(result))
  .catch(error => console.log('error', error));
      }, 300);
      }else if (_totalPrice > 99900){
      let variants = {
            "43324853485805":0,
            "43324857975021":1
            }
    ...
    function ship_update(result) {
     var orginalPrice = Math.round(result.original_total_price/100);
     var shiptotal_per =  (5 * orginalPrice)/100;
     var above_shipping_price = 499;
     var totalPrice1 = Math.round(result.total_discount/100) - above_shipping_price;
     var cartitemtotal = Math.round(result.total_price/100);
     var cartfreetotal = cartitemtotal + totalPrice1 ;
    var freetotal = above_shipping_price - cartitemtotal; 
     var ship_per = Math.round(shiptotal_per);
     var shiptotalPrice = Math.round(result.total_price/100 + 49);
     var charge = $('.free__cart__limit').attr('data-charge');
    if(shiptotalPrice <= 499){
      $('.free__cart__limit').text('₹'+charge);
      let tgt_new_cart_mssg = document.querySelector(".newcartmsg");
    let messageText = 'Shop for ₹'+ Math.round(freetotal)  +' more to unlock free shipping';
    // Check if the message is already present
    if (!tgt_new_cart_mssg.textContent.includes(messageText)) {
      let appendChild = document.createElement("div")
      appendChild.textContent = messageText;
      tgt_new_cart_mssg.innerHTML = '';
      tgt_new_cart_mssg.appendChild(appendChild);
    }
  setTimeout(function(){
    var cartTotalText = $(".payment_details").find(".cart__total__money").text('₹'+shiptotalPrice);
    }, 10);
      }
      else{
      $(".payment_details").find('.cart__total__money').text('₹'+result.total_price/100);
      $('.free__cart__limit').text('Free');
       let tgt_new_cart_mssg = document.querySelector(".newcartmsg");
    tgt_new_cart_mssg.innerHTML = '<div class="newcartmsg_text1">Hurray! You have Free shipping</div>'; 
      }
  }
 function free(){
  $.ajax({
